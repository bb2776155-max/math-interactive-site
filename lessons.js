...
const ALL_LESSONS = [
    ...
    }, // <-- 就是这里！刚才漏掉了这个至关重要的分隔逗号
    {
        "id": "lesson-trig-01",
        "title": "三角函数",
        "difficulty_tag": "1阶段", // 统一将原 tags 中的内容映射到你的标准字段上
        "stage_tag": "三角函数",
        ...
        "steps": [
            {
                ...
                "answer": "<div class='thick-content'>我们先追求得到尽可能多的信息，题目有 $\\cos \\beta$ 我们自然想知道 $\\sin\\beta$ 是多少。因为 $\\beta \\in \\left[0, \\frac{\\pi}{2}\\right]$，所以 $\\sin\\beta\\geq0$，所以 $\\sin\\beta=+\\sqrt{1-\\cos^2\\beta}=\\frac{\\sqrt 6}{3}$。同样我们 look 观察能否知道 $\\cos(\\alpha+\\beta)$ 是多少：<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>展开分析</span><span class='hidden'>因为 $\\alpha+\\beta\\in\\left[\\frac{\\pi}{2}, \\frac{3\\pi}{2}\\right]$，且已知 $\\sin(\\alpha+\\beta)=\\frac{1}{3} > 0$，可以进一步将范围精确锁定在第二象限 $\\alpha+\\beta\\in\\left[\\frac{\\pi}{2}, \\pi\\right]$。因此 $\\cos(\\alpha+\\beta)\\leq 0$，由此可以硬榨出：$$\\cos(\\alpha+\\beta)=-\\sqrt{1-\\sin^2(\\alpha+\\beta)}=-\\frac{2\\sqrt 2}{3}$$</span><br><br>现在已知了 $\\beta$ 以及 $\\alpha+\\beta$ 的正弦和余弦值。接下来我们能做什么？知道两个角的正弦和余弦值能做什么？<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>展开分析</span><span class='hidden'>既然我们要算的是关于 $\\alpha$ 的式子，自然想到利用已知角去拆凑未知角：$\\alpha = (\\alpha+\\beta) - \\beta$。那我们就可以算出这两个角相减的正弦值：$$\\sin\\alpha = \\sin(\\alpha+\\beta-\\beta) = \\sin(\\alpha+\\beta)\\cos\\beta - \\cos(\\alpha+\\beta)\\sin\\beta$$ 代入数据计算：$$\\sin\\alpha = \\frac{1}{3} \\cdot \\frac{\\sqrt{3}}{3} - \\left(-\\frac{2\\sqrt{2}}{3}\\right) \\cdot \\frac{\\sqrt{6}}{3} = \\frac{\\sqrt{3}}{9} + \\frac{4\\sqrt{3}}{9} = \\frac{5\\sqrt 3}{9}$$ 知道了 $\\sin\\alpha$，最后利用二倍角公式即可一枪干掉答案：$$\\cos2\\alpha = 1 - 2\\sin^2\\alpha = 1 - 2 \\cdot \\left(\\frac{5\\sqrt{3}}{9}\\right)^2 = 1 - 2 \\cdot \\frac{75}{81} = 1 - \\frac{50}{27} = -\\frac{23}{27}$$</span></div>"
            }
        ]
    }
];