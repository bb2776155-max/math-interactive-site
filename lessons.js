    ...
    },
    {
        ...
        "steps": [
            ...
            },
            {
                "id": "step_2",
                "question": "不含参同构：",
                "answer": "<div class='thick-content'>同学你需要有两个能力：1. 告诉你要找同构，你要能把它找出来。2. 考试题目不会告诉你要找同构，如何能看出来。我们先训练 1。<br><br>$\\mathrm{e}^{-x} - 2x - \\ln x \\geqslant 0$;<br>从目标出发，我们的目标是把它写成 $f(...)-f(\\text{...})\\geqslant 0$（或者 $f(...)\\geqslant f(\\text{...})$），ので所以应该是结构相同，至少是长得很对称的两组项。而原式是三个项，所以要拆，显然是拆 $2x$。把它写成 $\\mathrm{e}^{-x}-x-x-\\ln x=(\\mathrm{e}^{-x}-x)-(x+\\ln x)$，然后对照这两项的形式，再把它们写的更像一点，把前边那一项里边改成加法：$\\mathrm{e}^{-x}+(-x)-(x+\\ln x)$。原稿然后应该就能直接看出来。<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>若看不出来</span>：<span class='hidden'>你看它既然长得比较好看，你就直接大胆设 $f(x)=x+\\ln x$，然后看 $f(?)=\\mathrm{e}^{-x}+(-x)$</span><br><br>$x^2 \\mathrm{e}^x + \\ln x \\geqslant 0$.<br>观察到哪里不对称？那个 2 不对称，所以除掉一个 $x$，两边同除以 $x$ 得：$x \\mathrm{e}^x+\\frac{\\ln x}{x}$。<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>输入然后呢</span>？<span class='hidden'>一乘一除？注意我们的目标是 $f(...)-f(\\text{...})$，中间应该是减号不是加号，所以补个减号：$x\\mathrm{e}^x-(-\\frac{\\ln x}{x})$，环境然后使用这个常用变形：$-\\ln x=\\ln \\frac{1}{x}$（记住它），然后大胆设同构函数。</span></div><br><div class='thin-content font-semibold text-indigo-300'>【读薄】同构的构造分两步：1.通过等价变形把它变成比较规范比较对称比较好看的样子。2.设同构函数。</div>"
            },
            {
                "id": "step_3",
                "question": "含参同构的分类：",
    ...
    },
    {
        ...
        "steps": [
            ...
            },
            {
                ...
                "answer": "<div class='thick-content'>解：<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>先大概画一下 $f(x)$长啥样……</span><span class='hidden'><span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>$\\ln(-x-1)$ 怎么画？</span><span class='hidden'>先画 $\\ln x$……<br><br>$$\\begin{tikzpicture}[scale=0.8] \\draw[->] (-1,0) -- (4,0) node[right] {$x$}; \\draw[->] (0,-3) -- (0,2) node[above] {$y$}; \\node[below left] at (0,0) {$0$}; \\draw[domain=0.08:3.5, smooth, variable=\\x, blue, thick] plot ({\\x}, {ln(\\x)}); \\node[blue, right] at (2.5,1) {$y=\\ln x$}; \\filldraw (1,0) circle (1.5pt) node[below right] {$1$}; \\end{tikzpicture}$$<br><br>，那么 $\\ln (-x)$ 的图像就是……<br><br>$$\\begin{tikzpicture}[scale=0.8] \\draw[->] (-4,0) -- (1,0) node[right] {$x$}; \\draw[->] (0,-3) -- (0,2) node[above] {$y$}; \\node[below right] at (0,0) {$0$}; \\draw[domain=-3.5:-0.08, smooth, variable=\\x, blue, thick] plot ({\\x}, {ln(-\\x)}); \\node[blue, left] at (-2.5,1) {$y=\\ln(-x)$}; \\filldraw (-1,0) circle (1.5pt) node[below left] {$-1$}; \\end{tikzpicture}$$<br><br>。那么 $\\ln(-x-1)$ 是 $\\ln(-x)$ 的图像往左移还是往右移？<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>对于一个函数 $g(x)$，遵循左加右减，$g(x+a)$ 是往左移 $a$，$g(x-a)$ 是往右移 $a$。对于 $g(x)=\\ln(-x)$，$\\ln(-x-1)=\\ln(-(x+1))=g(x+1)$，所以是往左移 1。所以判断方法不是依据括号里加一还是减一，比如 $g(x)=\\ln(x^2)$，$\\ln(x^2+1)$ 的括号里加了 1，但是它根本不是 $\\ln(x^2)$ 的平移，因为它没法写成 $g(x+a)$ 的形式。<br><br>所以我们画出 $f(x)$ 的图像……<br><br>$$\\begin{tikzpicture}[scale=0.8] \\draw[->] (-5,0) -- (2,0) node[right] {$x$}; \\draw[->] (0,-3) -- (0,4) node[above] {$y$}; \\node[below left] at (0,0) {$0$}; \\draw[domain=-4.5:-1.05, smooth, variable=\\x, blue, thick] plot ({\\x}, {ln(-\\x-1)}); \\draw[dashed] (-1,-3) -- (-1,4); \\node[below left] at (-1,0) {$-1$}; \\filldraw (-2,0) circle (1.5pt) node[below left] {$-2$}; \\draw[domain=-1:1.2, smooth, variable=\\x, red, thick] plot ({\\x}, {2*\\x+1}); \\filldraw (-1,-1) circle (1.5pt) node[right] {$(-1,-1)$}; \\filldraw (0,1) circle (1.5pt) node[right] {$1$}; \\node[blue] at (-3,1.2) {$y=\\ln(-x-1)$}; \\node[red] at (0.8,3.2) {$y=2x+1$}; \\end{tikzpicture}$$<br><br>我们和上题一样算出 $f(f(x))$ 的单调性。$f(x)$ 属于 $f$ 的单减区间等价于 $f(x)\\in(-\\infty,-1]$。<br><br>$x < -1 : \\ln(-x-1) \\le -1 \\Leftrightarrow \\mathrm{e}^{\\ln(-x-1)} \\le \\mathrm{e}^{-1} \\Leftrightarrow -x-1 \\le \\mathrm{e}^{-1} \\Leftrightarrow -1-\\frac{1}{\\mathrm{e}} \\le x \\therefore -1-\\frac{1}{\\mathrm{e}} \\le x < -1$<br><br>$x \\ge -1 : 2x+1 \\le -1 \\Leftrightarrow x \\le -1 \\therefore x = -1$<br><br>所以 $f(x)$ 属于 $f$ 的单减区间等价于 $x\\in[-1-\\frac{1}{\\mathrm{e}},-1]$。所以 $f(x)$ 属于 $f$ 的单增区间等价于 $x\\in(-\\infty,-1-\\frac{1}{\\mathrm{e}}]\\cup[-1,+\\infty)$。由此得到 $f(f(x))$ 的单调性如下表……<br><br>$$\\begin{array}{|c|c|c|c|} \\hline x & (-\\infty, -1-\\frac{1}{\\mathrm{e}}] & [-1-\\frac{1}{\\mathrm{e}}, -1] & [-1, +\\infty) \\\\ \\hline f(x) & \\searrow & \\nearrow & \\nearrow \\\\ \\hline f(x)\\text{所属区间} & f\\text{的单增区间} & f\\text{的单减区间} & f\\text{的单增区间} \\\\ \\hline f(f(x)) & \\searrow & \\searrow & \\nearrow \\\\ \\hline f(x)\\text{的范围} & [-1,+\\infty) & (-\\infty,-1] & [-1,+\\infty) \\\\ \\hline f(f(x))\\text{的值域} & [-1,+\\infty) & [-\\1,+\\infty) & [-1,+\\infty) \\\\ \\hline \\end{array}$$</span></span></span></div>"
            }
        ]
    }
];