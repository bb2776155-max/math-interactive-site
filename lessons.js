...
<p class='lesson-paragraph'><span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>读薄</span><span class='hidden'>要意识到$e^{2x}+e^{-2x}$这种形式的特别之处，它俩的乘积为常数，所以它可以转化成$(e^x+e^{-x})^2-2$。如果是$e^{2x}-e^{-2x}$也可以转化成$(e^x-e^{-x})(e^x+e^{-x})$<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>。</span><span class='hidden'>则可更通用地（不依赖$e^x\\cdot e^{-x}等于常数$）转化为：$e^{2x}-e^{-2x}=(e^x+e^{-x})\\cdot(e^x-e^{-x})$。</span></span></p>
