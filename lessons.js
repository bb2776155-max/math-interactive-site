...
<div class='thin-content mt-3'>【读薄】要意识到$e^{2x}+e^{-2x}$这种形式的特别之处，它俩的乘积为常数，所以它可以转化成$(e^x+e^{-x})^2-2$。如果是$e^{2x}-e^{-2x}$<span class='math-inline-trigger' onclick='toggleInlinePPT(this)'>也可以转化成</span><span class='hidden'>$(e^x-e^{-x})(e^x+e^{-x})$。则可更通用地（不依赖$e^x\\cdot e^{-x}等于常数$）转化为：$e^{2x}-e^{-2x}=(e^x+e^{-x})\\cdot(e^x-e^{-x})$。</span></div>
...
<div class='thin-content mt-3'>【读薄】这题第一问对题目条件进行使用即可；第二问观察函数特殊形式。法二训练的是更迅速的感觉，法一训练的是假设你看到$\\frac{10+(e^x+e^{-x})^2-2}{2(e^x+e^{-x})}$也要能看出来怎么化简。</div>
</div>
</div></div>`
            }
        ]
